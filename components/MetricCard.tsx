...
export default function MetricCard({ metric }: MetricCardProps) {
  ...
  return (
    <div className={`bg-gradient-to-br ${config.bgGradient} backdrop-blur-sm rounded-xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-6 hover:shadow-xl transition-all duration-300 hover:scale-105 group`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 bg-gradient-to-br ${config.gradient} rounded-lg shadow-md group-hover:shadow-lg transition-shadow`}>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className={`text-3xl font-bold bg-gradient-to-r ${config.gradient} bg-clip-text text-transparent`}>
            {formatValue(parseFloat(metric.avg_value))}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-200/50 dark:border-gray-700/50">
          <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3 backdrop-blur-sm">
            ...
            <p className="text-sm font-bold text-gray-900 dark:text-white">
              {formatValue(parseFloat(metric.total_value))}
            </p>
          </div>
          <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 pt-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>Updated {format(new Date(metric.latest_timestamp), 'MMM d, HH:mm')}</span>
        </div>
      </div>
    </div>
  );
